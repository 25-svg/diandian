...
把直播负责人完成的逐字稿校正、高光复盘和辅稿候选，以可审核、可追溯、可重建的方式沉淀到 Obsidian，并让后续 ASR 纠错、高光分析和母稿编排能够读取已审核知识。
