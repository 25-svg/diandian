...
|------|-------------------|------------|--------------|
| **公司录播** | 能（罗盘 + 订单，legacy 模式可用） | **整场 AI 话术质检**（不做成交片段 Tab） | `{主播名}知识库/`（`视频/成交`、`话术`、`分析建议`） |
| **竞品录播** | **不能** | 用**母稿当尺子**拆竞品话术 | 先 `03-直播案例`；公司成交走主播知识库，不与竞品目录混用 |
...
| 5 | （可选）**申请采纳进母稿** | 候选辅稿队列 → 人审后进入对应 `{主播名}知识库`（历史版本可能仍在 `10-企业母稿`） |
...
历史完整正文见知识库：`10-企业母稿/MS-BATCH-4/V1.0/README.md`（新发布请到对应 `{主播名}知识库/话术/`）

---

## 七、首次建立企业母稿（一次性）

> **整场导入直接进母稿已废弃。** 正确路径：成交对齐 → 精炼话术 → 切片复盘验证 → 人工确认 → Clip「母稿样本批次」→ 发布到 `{主播名}知识库`。

```mermaid
flowchart LR
  V1[成交精炼样本] --> V2[Clip 创建批次并填写主播名]
  V2 --> V3[生成并核对草稿]
  V3 --> V4[发布到主播知识库]
  V4 --> V5[视频/成交 · 话术 · 分析建议]
```
...
| 步骤 | 操作 |
|:----:|------|
| 1 | 准备已人工确认的成交精炼话术样本（非整场视频） |
| 2 | Clip → **创建母稿样本批次**，填写真实主播名（样本来源） |
| 3 | 生成最终草稿并核对固定原话 / 证据 |
| 4 | **审核通过并发布** → vault 出现 `{主播}知识库/视频/成交`、`话术`、`分析建议` |
| 5 | 视频仅为路径引用，不会把 MP4 拷进 vault |

历史正文若仍在 `10-企业母稿/MS-BATCH-*/`，可继续只读对照；**新发布写入主播知识库**。
...
| `{主播名}知识库/话术/` 等 | 新发布的公司成交话术 / 建议 / 视频引用 |
| `10-企业母稿/MS-BATCH-4/V1.0/` | 历史企业标准正文（只读兼容） |
