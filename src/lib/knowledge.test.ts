...
assert.deepEqual(knowledgeStatusPresentation({
  ...
  activeCount: 0, eligibleCount: 0, pendingReviewCount: 0, ignoredCount: 0,
...
assert.deepEqual(knowledgeStatusPresentation({
  ...
  activeCount: 547, eligibleCount: 2, pendingReviewCount: 533, ignoredCount: 12,
  restrictedCount: 0, errorCount: 0,
}), {
  tone: "warning",
  label: "有 533 张卡片等待审核",
  detail: "知识卡 535 张：可用 2，待审核 533；另忽略 12 篇说明文档",
});

assert.deepEqual(knowledgeStatusPresentation({
  connected: true, vaultPath: "C:/Vault", status: "degraded", lastSyncedAt: "2026-07-22 10:00:00",
  activeCount: 12, eligibleCount: 8, pendingReviewCount: 1, ignoredCount: 0,
  restrictedCount: 1, errorCount: 2,
}), {
  tone: "warning",
  label: "有 2 个格式问题需要处理",
  detail: "知识卡 12 张：可用 8，待审核 1，受限 1，异常 2",
});

assert.equal(knowledgeSyncSummaryText({
  vaultPath: "C:/Vault", inserted: 3, updated: 2, unchanged: 5,
  deactivated: 1, eligibleCount: 7, pendingReviewCount: 1, ignoredCount: 1,
