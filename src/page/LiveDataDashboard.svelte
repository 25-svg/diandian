<script lang="ts">
  ...
  import { CalendarDays, Download, FolderOpen, Loader2, RefreshCw, Upload } from "lucide-svelte";
  ...
  async function queryCompassSessions() {
    try {
      ...
      compassQueue = [];
      message = "正在打开抖音罗盘；首次使用请扫码登录。";
      await invoke("query_compass_live_sessions", { targetDate: compassDate });
  ...
  async function startCompassDownloads() {
    try {
      ...
      message = `正在串行下载并导入 ${compassDate} 的全部直播场次。`;
      await invoke("start_compass_live_downloads", { targetDate: compassDate });
    } catch (error) {
      compassRunning = false;
  ...
  function handleCompassProgress(progress: CompassBrowserProgress) {
    ...
    compassQuerying = !["sessions-found", "failed", "batch-finished"].includes(progress.status);
...
<PageShell title="直播数据大屏" subtitle="数据来自官方“整场数据下载” XLSX；未提供字段不做推算。">
      ...
      <div class="compass-queue">
        {#each compassQueue as item (item.sessionKey)}
          <article class:success={item.status === "imported" || item.status === "skipped"} class:error={item.status === "failed"}>
            <div><strong>{new Date(item.session.startedAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}–{new Date(item.session.endedAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</strong><span>{item.session.shopName}</span></div>
  ...
  {#if sessions.length}
    <select class="mac-field session-select" value={detail?.session.id} on:change={(event) => refresh(Number(event.currentTarget.value))}>
...
<style>
  ...
  .compass-controls { margin-top: 14px; }
  ...
  .session-select { margin-top: 0; min-width: 320px; width: auto; height: 34px; }
  .cards {
